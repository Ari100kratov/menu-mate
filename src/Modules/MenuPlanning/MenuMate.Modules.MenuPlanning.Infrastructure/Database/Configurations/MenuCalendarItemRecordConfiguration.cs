...
internal sealed class MenuCalendarItemRecordConfiguration : IEntityTypeConfiguration<MenuCalendarItemRecord>
{
    public void Configure(EntityTypeBuilder<MenuCalendarItemRecord> builder)
    {
        ...
        builder.ToTable("menu_calendar_items");
        ...
        builder.HasIndex(item => new { item.OwnerUserId, item.Date, item.MealSlotId, item.Position });
        builder.HasIndex(item => new { item.OwnerUserId, item.Date });
