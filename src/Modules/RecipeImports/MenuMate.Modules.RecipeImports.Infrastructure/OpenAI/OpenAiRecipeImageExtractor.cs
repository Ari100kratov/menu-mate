        ...
        try
        {
            ...
            [
                ChatMessageContentPart.CreateTextPart(
                    ...
                    - Если на изображениях есть отдельные советы, примечания, варианты, предупреждения, рекомендации по подаче, хранению или замене ингредиентов, перенеси их в recipe.advice. Не дублируй в советы описание блюда, ингредиенты или шаги и не придумывай информацию. Если таких данных нет, верни advice=null.
                    - Нормализуй текст советов: не копируй переносы, возникшие только из-за ширины карточки или строки на изображении. Ставь пустую строку только между явно отдельными абзацами, пунктами или смысловыми блоками. Начало с заглавной буквы и знак конца предложения — лишь дополнительный признак, а не самостоятельная причина переноса; если граница неочевидна, соедини фрагменты пробелом.
                ...
                ChatMessageContentPart.CreateTextPart(
                    ...
                    - Категория яиц — значимая характеристика: «яйцо C0» и «яйцо C1» — разные продукты. Сохраняй категорию в productName, ставь category="Eggs" и не объединяй такие яйца между собой.
                    - Если ингредиенты вынесены под заголовок отдельной части блюда («Для соуса», «Соус», «Тесто», «Начинка»), всё равно верни их в общий список, но добавь в comment каждого такого ингредиента назначение: «для соуса», «для теста» и так далее. Сохрани существующий комментарий, объединив части через «; ». Не добавляй назначение, если заголовок не виден или неясен.
                    ...
                    - Обыкновенные и смешанные дроби преобразуй в десятичное количество: «1/3 ст. л. соли» => amount=0.33, unit="Tablespoon"; «1 1/2 ч. л.» => amount=1.5, unit="Teaspoon". Знаменатель дроби никогда не считай отдельным количеством.
                    ...
                    - Если количество порций явно указано, перенеси его в servings без изменений. Иначе оцени правдоподобное целое от 1 до 100 по типу блюда, объёму и массе ингредиентов; значение 1 используй только для очевидно индивидуального блюда. Если порции рассчитаны, добавь предупреждение «Количество порций рассчитано приблизительно — проверьте его.».
                    """),
                ChatMessageContentPart.CreateTextPart(
                    $"""
