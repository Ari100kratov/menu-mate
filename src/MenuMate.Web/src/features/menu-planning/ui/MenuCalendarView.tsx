...
import { formatDayHeading, getRangeDates, type MenuDateRange } from "@/features/menu-planning/model/menu-calendar"
  ...
  return (
    <section className="space-y-3">
      ...
      {getRangeDates(range).map((date) => (
        <article key={date} className="bg-secondary/35 border-primary/10 space-y-3 rounded-2xl border p-3">
          <h2 className="type-subsection-title text-secondary-foreground px-1">{formatDayHeading(date)}</h2>
          <div className="space-y-2">
            {mealSlots.map((mealSlot) => {
              ...
              return (
                <section key={mealSlot.id} className="bg-card/90 border-primary/10 space-y-2 rounded-xl border p-3 shadow-sm">
                  <header className="flex items-center justify-between gap-2">
                    ...
                    <Button
                      ...
                      aria-label={isPlacementMode ? undefined : `Добавить блюдо в ${mealSlot.name}`}
                      title={isPlacementMode ? undefined : `Добавить блюдо в ${mealSlot.name}`}
                      disabled={isPending}
                      onClick={() => { onAdd(date, mealSlot.id) }}
                    >
                      <Plus />
                    ...
                    <div className="space-y-2">
                      {slotItems.map((item) => (
                        <MenuItemRow
                          ...
                          mealSlots={mealSlots}
                          isPending={isPending}
                          onUpdate={(request) => { onUpdate(item.id, request) }}
