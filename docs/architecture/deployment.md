...
Официальный образ PostgreSQL 18 хранит данные в version-specific каталоге `/var/lib/postgresql/18/docker` и рекомендует монтировать родительский `/var/lib/postgresql`. Compose уже использует этот layout. Том, созданный образами PostgreSQL 17 и ниже с mount `/var/lib/postgresql/data`, нельзя подключать к 18 без отдельного major upgrade через dump/restore или `pg_upgrade`: простая смена image tag или mount path не переносит кластер.
