...
Фильтры хранятся в `localStorage` отдельно для «Моей библиотеки» и «Каталога». Сохраняются последняя вкладка, текст запроса, категория и режим «Только избранные». Выбор рецепта внутри календаря имеет отдельный набор сохранённых фильтров, чтобы рабочий поиск в меню не перезаписывал состояние основного списка.
